...
export default function Navbar() {
  ...
  return (
    <>
      ...
      ========================================= */}
      <nav 
        className={`fixed z-[110] font-clash flex justify-between items-center px-4 py-2 shadow-sm transition-all duration-300 ease-in-out left-0 right-0 mx-auto w-[calc(100%-2rem)] md:w-[calc(100%-4rem)] max-w-5xl rounded-[2rem]
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center px-4 pt-24 pb-5">
          ...
          {/* Menu Box */}
          <div className="relative z-10 w-full font-clash max-w-5xl bg-white rounded-2xl p-6 flex flex-col gap-5">
            
            {/* PRODUCTS SECTION - INSIDE LIGHT GREY CARD */}
            <div className="bg-gray-50 rounded-xl p-4">
              <h4 className="text-[10px] font-medium tracking-wider text-gray-400 uppercase mb-2">
                OUR PRODUCTS
              </h4>
              <div className="flex flex-col gap-2">
                <Link 
                  onClick={() => setIsMenuOpen(false)} 
                  to="/" 
                  className="text-sm text-gray-900 hover:text-black transition"
                >
                  Auvra Core
                </Link>
                <Link 
                ...
                <span className="text-sm text-gray-400">
                  Auvra Node <span className="text-[10px] text-gray-400 ml-2">Coming soon</span>
                </span>
              </div>
            </div>

            {/* ASK LENS AI */}
            <a href="#" className="text-sm text-gray-900 hover:text-black transition">
              Ask Lens AI
            </a>

            {/* COMPANY */}
            <Link onClick={() => setIsMenuOpen(false)} to="/about" className="text-sm text-gray-900 hover:text-black transition">
              Company
            </Link>

            {/* BLOG */}
            <Link onClick={() => setIsMenuOpen(false)} to="/blog" className="text-sm text-gray-900 hover:text-black transition">
              Blog
            </Link>
            ...
            <div className="pt-2">
              <Link 
                ...
                className="relative block overflow-hidden rounded-xl bg-gray-900 transition-all duration-300 hover:scale-[1.01] w-full text-left"
              >
                <img 
                  src="/IMG_inst.JPG" 
